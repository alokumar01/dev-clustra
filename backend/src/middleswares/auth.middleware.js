...
export const protect = async (req, res, next) => {
    try {
        ...
        // We used 'sub' in generateToken, so we access it here
        const user = await User.findById(decoded.sub).select("-passwordHash -emailVerifyHash -emailVerifyExpires -__v");
        
        if (!user) {
            throw new ApiError(401, "The user belonging to this token no longer exists.", "USER_NOT_FOUND");
        }

        // 4. Check if account is still active (The moderation check we discussed!)
